...
<template>
	<view class="join-images">
		<drag v-model="imgUrlArr">
		</drag>
		<!-- 吸底的两个按钮 -->
		<view class="bottom-btns" v-show="successUpload">
			<view class="bottom-btns-item" @tap="horizonConcat">
				横向拼接
				<icons type="icon-add" size="40" color="#fff"></icons>
			</view>
			<view class="bottom-btns-item" @tap="verticalConcat">
				竖向拼接
				<icons type="icon-save" size="40" color="#fff"></icons>
			</view>
		</view>
								...
								if(i===this.imgUrlArr.length-1){
									...
									this.$nextTick(()=>{
										this.drawImgs.forEach((item,inx)=>{
											...
											const previewImgsWidth = this.drawImgs.slice(0,inx).map((item)=>{
												return item.width;
											})
